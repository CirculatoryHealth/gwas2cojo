...
- **`fix_bc_michailidou.sh`**: CRLF strip added for robustness. Output headers changed from generic pipeline aliases (`SNPID CHR POS NEA EA EAF BETA SE P`) to the original source column names (`var_name chr position_b37 a0 a1 bcac_onco_icogs_gwas_*`). ⚠ `gwas_process.py` may not auto-detect these non-standard column names — verify alias resolution during preprocess or add column mappings via `EXTRA_FLAGS`. Rerun `fix_bc_michailidou.sh` before resubmitting `BC_EUR_Michailidou2017`, `BC_EUR_ERpos_Michailidou2017`, `BC_EUR_ERneg_Michailidou2017`.
